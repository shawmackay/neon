	...
	public void run() {
		...
		AsyncHandler myHandler = new AsyncHandler() {
			public void replied(Object handback, Method handbackMethod, boolean isException) {
				System.out.println("Generated AsyncHandler has received a reply: " + handback);
				if (isException) {
					System.out.println(((Exception) handback).getMessage());

					((Exception) handback).printStackTrace();
				}
			}
		};
