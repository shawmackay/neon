...
/**
 * @author calum
 *@author calum 05 March 2002
 * 0.9community
 */
public class AthenaMonitorGrid extends JComponent {
	...
	public void paint(Graphics g) {
		...
		/*
		 * if (image!=null){ *int imheight =
		 * image.getHeight((java.awt.image.ImageObserver) this); int imwidth =
		 * image.getHeight((java.awt.image.ImageObserver) this); double
		 * aspectratio =(double)imheight / (double) imwidth;
		 * g.drawImage(image.getScaledInstance(getWidth(), (int)
		 * ((double)getWidth()*aspectratio),image.SCALE_FAST),0,0,(java.awt.image.ImageObserver)
		 * this);
		 * 
		...
		if (hashes.length > 0) {
			...
			// g2.drawLine(xpos, ypos, xpos+(xinc*hashes.length),
			// ypos+(yinc*titles.length));
			// g2.drawLine(xpos, ypos, xpos-(xinc*5), ypos+(yinc*titles.));
			// g2.drawLine(xpos-(xinc*5), ypos+(yinc*5),
			// xstart+xinc,ypos+(yinc*10));
			// g2.drawLine(xstart+xinc,ypos+(yinc*10), xpos+(xinc*5),
			// ypos+(yinc*5));
			...
			// These arerelative from the far end, i.e. 3/4 of f the way back
			// from 100 is 1/4 or 25
